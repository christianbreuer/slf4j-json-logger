...
public abstract class AbstractJsonLogger implements JsonLogger {
	...
	public AbstractJsonLogger(org.slf4j.Logger slf4jLogger, String dateKey,
	...
	public JsonLogger field(String key, Supplier value) {
		try {
			// in the rare case that the value passed is null, this method will
			// be selected as more specific than the Object
			// method. Have to handle it here or the value.get() will
			// NullPointer
			if (value == null) {
	...
	protected String formatMessage(String level) {
		...
		jsonObject.add("thread_name",
				gson.toJsonTree(Thread.currentThread().getName()));
		...
		if (includeLoggerName) {
			jsonObject.add("logger_name",
					gson.toJsonTree(slf4jLogger.getName()));
		}

		try {
			jsonObject.add(dateKey,
					gson.toJsonTree(getCurrentTimestamp(formatter)));
		} catch (Exception e) {
			jsonObject.add(dateKey, gson.toJsonTree(formatException(e)));
		}
	...
	/**
	 * Some contention over performance of Thread.currentThread.getStackTrace()
	 * vs (new Exception()).getStackTrace() Code in Thread.java actually uses
	 * the latter if 'this' is the current thread so we do the same
	 *
	 * Remove the top two elements as those are the elements from this logging
	 * class
	 */
