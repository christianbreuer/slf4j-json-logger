...
public class BasicLoggingTests {
	...
	public void messageEnabled() {
		...
		Logger logger = new Logger(slf4jLogger, dateKey, formatter, true);

		logger.info().message("My message").log();

		ArgumentCaptor<String> messageCaptor = ArgumentCaptor
	...
	public void allCollections() {
		...
		logger.trace().message("Report executed").map("someStats", map)
				.list("customers", list).field("year", "2016").log();
	...
	public void fieldOverwritesCategory() {
		...
		logger.warn().message("This gets overwritten")
				.field("message", "This wins").log();
	...
	public void lambdas() {
		...
		logger.error().message(() -> "Something expensive").log();
	...
	public void includesLoggerName() {
		...
		Logger logger = new Logger(slf4jLogger, dateKey, formatter, true);
		logger.info().message("something").log();
	...
	public void doesNotIncludeLoggerName() {
		String expectedLoggerNameKey = "\"logger_name\":";
		...
		Logger logger = new Logger(slf4jLogger, dateKey, formatter, false);
		logger.info().message("something").log();
	...
	@Test
	public void includesTimestampAttributeName() {
		String expectedLoggerName = "\"@testtimestampName\"";

		org.slf4j.Logger slf4jLogger = mock(org.slf4j.Logger.class);
		when(slf4jLogger.isInfoEnabled()).thenReturn(true);
		when(slf4jLogger.getName()).thenReturn(this.getClass().getName());

		Logger logger = new Logger(slf4jLogger, "@testtimestampName", formatter,
				true);
		logger.info().message("something").log();
